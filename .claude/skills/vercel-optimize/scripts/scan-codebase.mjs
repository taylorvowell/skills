...
// Route regexes and include-globs are POSIX-separated; relative() yields '\' on
// Windows. Normalize every emitted relative path so matching is platform-agnostic.
...
async function enrichRoutesWithWorkspaceImports(routes, scanRootDir, resolver, monorepoRoot) {
  ...
  for (const r of routes) {
    ...
    r.workspaceImports = resolved
      ...
      .map((abs) => toPosix(relative(monorepoRoot, abs)));
  }
}
...
async function collectFiles(root) {
  ...
  for (const e of entries) {
    ...
    try {
      ...
      out.push({ path: toPosix(relative(root, full)), content });
    } catch {}
  }
  return out;
}
