...
// On Windows the `vercel` CLI is a `.CMD`/`.cmd` shim. Node's execFile (no shell,
// no PATHEXT) can't find it, and modern Node refuses to spawn a `.cmd` without a
// shell. We resolve the shim's full path and invoke it via cmd.exe with
// MS-C-runtime arg quoting so OData filter args (`http_status ge '500'`) survive
// intact. Non-Windows behavior is unchanged.
let _vercelWinPath;
function resolveVercelWinPath() {
  if (_vercelWinPath !== undefined) return _vercelWinPath;
  const exts = ['.CMD', '.cmd', '.exe', '.bat', ''];
  const dirs = (process.env.PATH || '').split(delimiter).filter(Boolean);
  for (const dir of dirs) {
    for (const ext of exts) {
      const candidate = join(dir, `vercel${ext}`);
      if (existsSync(candidate)) { _vercelWinPath = candidate; return _vercelWinPath; }
    }
  }
  _vercelWinPath = null;
  return _vercelWinPath;
}

// MS C-runtime argv quoting (handles spaces, embedded quotes, trailing backslashes).
...
function exec(file, args = [], opts = {}) {
  if (!IS_WIN || file !== 'vercel') return execRaw(file, args, opts);
  const cmdPath = resolveVercelWinPath();
  if (!cmdPath) return execRaw(file, args, opts); // surfaces VERCEL_NOT_INSTALLED upstream
  const line = [winQuoteArg(cmdPath), ...args.map(winQuoteArg)].join(' ');
  return execRaw(process.env.ComSpec || 'cmd.exe', ['/d', '/s', '/c', `"${line}"`], {
    ...opts,
