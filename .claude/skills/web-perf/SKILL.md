---
name: web-perf
description: Profiles and measures web page load performance using the Chrome DevTools MCP. Records a real performance trace, measures Core Web Vitals (LCP, INP, CLS) and supplementary metrics (FCP, TBT, Speed Index), and identifies render-blocking resources, network dependency chains, layout shifts, caching issues, and accessibility gaps. Use when asked to profile or measure page load, analyze Core Web Vitals or Lighthouse scores, find render-blocking resources or layout shifts, or debug/optimize site speed via Chrome DevTools traces. Biases towards retrieval from current documentation over pre-trained knowledge.
---
