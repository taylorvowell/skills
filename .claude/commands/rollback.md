...
1. List the 10 most recent `checkpoint-*` tags (covers both the `checkpoint-<trackId>-step-*` form and any legacy `checkpoint-step-*`), sorted newest first, showing each tag's track, step number, and timestamp
...
6. Update `_STATUS.json` via `progress-tracker` to mark every step *after* the checkpoint's step number as `not-started` — not only `completed` steps but any `in-progress`, `blocked`, or `skipped` one (their code is gone after the reset), removing the matching `blockers[]` / `skipped[]` entries — and set `currentStep` back to the checkpoint's step
